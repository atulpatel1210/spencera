...
document.addEventListener('DOMContentLoaded', function() {
    ...
    finish_id: row.querySelector('.design-finish-id').value
        ...
        // Validate that sum of design quantities matches calculated pallet quantity
        const designQtyErrorDiv = palletItemDiv.querySelector('.design-qty-error');
        if (calculatedPalletQty > 0 && sumOfDesignQuantities !== calculatedPalletQty) {
    ...
    function addDesignToPallet(palletItemDiv) {
        ...
        const designQty = parseInt(designQuantityInput.value);
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        if (!seenDesigns.has(item.design_detail.id)) {
                            uniqueDesigns.push({ id: item.design_detail.id, name: item.design_detail.name });
                    ...
                    data.forEach(item => {
                        if (!seenSizes.has(item.size_detail.id)) {
                            uniqueSizes.push({ id: item.size_detail.id, size_name: item.size_detail.size_name });
                    ...
                    data.forEach(item => {
                        if (!seenFinishes.has(item.finish_detail.id)) {
                            uniqueFinishes.push({ id: item.finish_detail.id, finish_name: item.finish_detail.finish_name });
