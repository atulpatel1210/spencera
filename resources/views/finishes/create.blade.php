            ...
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-header bg-white py-3 px-4 border-bottom d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-bold text-primary">
                        <i class="fas fa-fill-drip me-2"></i> {{ isset($finish) ? 'Edit' : 'Add' }} Finish
                    </h5>
                ...
                <div class="card-body p-4">
                    <form action="{{ isset($finish) ? route('finishes.update', $finish) : route('finishes.store') }}" method="POST" class="needs-validation" novalidate>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-5">
                            <a href="{{ route('finishes.index') }}" class="btn btn-light border rounded-pill px-4 fw-medium text-secondary">
                                <i class="fas fa-times-circle me-1"></i> Cancel
                            </a>
                            <button type="submit" class="btn btn-primary rounded-pill px-5 shadow-sm fw-bold">
                                <i class="fas fa-save me-1"></i> {{ isset($finish) ? 'Update' : 'Save Finish' }}
                            </button>
                        </div>
                    </form>
                </div>
